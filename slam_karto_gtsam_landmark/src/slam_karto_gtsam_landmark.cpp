...
private:
  ...
  void publishOptimizedOutput(GTSAMSolver *solver_);
  ...
  ros::Publisher optimoutput_publisher_;
  ros::Publisher sstm_;
                         ...
                         marker_count_(0)
{
  ...
  if (!private_nh_.getParam("robot_namespace", robot_namespace_)) {
    robot_namespace_ = "";
  }
  ...
  optimoutput_publisher_ = node_.advertise<slam_karto_gtsam_landmark::OptimizationOutput>("optimized_graph_output", 100);
...
void SlamKarto::publishGraphVisualization()
{
  ...
  std::cout << "CHECKPOINT received correctedGraphNodes_" << std::endl;
  visualization_msgs::MarkerArray marray;

  visualization_msgs::Marker m;
  // m.header.frame_id = "map";
  // m.header.frame_id = robot_namespace_ + "/map";
  m.header.frame_id = "new_" + robot_namespace_ + "_map";
  ...
  // edge.header.frame_id = robot_namespace_ + "/map";
  m.header.frame_id = "new_" + robot_namespace_ + "_map";
  ...
  for (uint i = 0; i < graph_nodes.size(); i++)
  {
    ...
    std::cout << "CHECKPOINT added graphnodes" << std::endl;
    id++;
  }
...
void SlamKarto::publishOptimizedOutput(GTSAMSolver *solver_) {
  
  using namespace gtsam;
  slam_karto_gtsam_landmark::OptimizationOutput optim_out_msg;

  gtsam::Values result;
  gtsam::NonlinearFactorGraph graph;
  std::vector<int> observed_key_ids;
  solver_->getOptimizedResult(result);
  solver_->getGTSAMGraph(graph);
  solver_->getObservedKeyIds(observed_key_ids);
  // solver_->printOptimizedResult();

  result.print("PUBLISH OPTIMIZATION OUTPUT result:\n"); // prints Value, and post-optimization pose/point
  ...
  for (const auto& value : result) {
    gtsam::Symbol key = value.key;
    gtsam::Matrix marginalCovariance = marginals.marginalCovariance(key); // Matrix

    // for robot POSE (x, y, theta) ____________________
    if (value.value.dim() == 3) {
        gtsam::Pose2 pose2 = value.value.cast<Pose2>();
        // ss << "Optimized Position (x, y, theta): (" << pose2.x() << ", " << pose2.y() <<", " << pose2.theta() <<")";
        // push back robot pose
        optim_out_msg.poses.push_back(pose2.x()); 
        optim_out_msg.poses.push_back(pose2.y());
        optim_out_msg.poses.push_back(pose2.theta());

        // push back robot pose marginal covariance in row-major order
        for (int i = 0; i < 3; ++i) {
          for (int j = 0; j < 3; ++j) {
              optim_out_msg.pose_covariances.push_back(marginalCovariance(i, j));
          }
        }
        // push back pose node graph key index
        optim_out_msg.keys_poses.push_back(key.index());
      }

      else if (value.value.dim() == 2) {
        gtsam::Point2 point2 = value.value.cast<Point2>();
        // ss << "Optimized Position (x, y): (" << point2.x() << ", " << point2.y() << ")";
        // push back map feature point location
        optim_out_msg.points.push_back(point2.x());
        optim_out_msg.points.push_back(point2.y());

        // push back robot pose marginal covariance in row-major order
        optim_out_msg.point_covariances.push_back(marginalCovariance(0,0));
        optim_out_msg.point_covariances.push_back(marginalCovariance(0,1));
        optim_out_msg.point_covariances.push_back(marginalCovariance(1,0));
        optim_out_msg.point_covariances.push_back(marginalCovariance(1,1));

        // push back pose node graph key index
        optim_out_msg.keys_points.push_back(key.index());
      }
  }

  for (auto &id_element : observed_key_ids)
  {
    optim_out_msg.keys_observed_points.push_back(id_element);
  }

  optim_out_msg.header.stamp = ros::Time::now();
  optimoutput_publisher_.publish(optim_out_msg);
}
...
void SlamKarto::laserCallback(const sensor_msgs::LaserScan::ConstPtr &scan)
{
  ...
  if (addScan(laser, scan, odom_pose))
  {
    ...
    publishGraphVisualization(); // uncomment to see visualization markers

    if (optim_init_counter_ == 5) {
      publishOptimizedOutput(solver_);
      optim_init_counter_ = 0;
    }
    optim_init_counter_++;
                        ...
                        karto::Pose2 &karto_pose)
{
  ...
  // Create a vector of doubles for karto. readings is the vector of doubles with range readings
  ...
  range_scan->SetLaserScanAngleMin(scan->angle_min);
  range_scan->SetLaserScanAngleIncrement(scan->angle_increment);
  ...
  if ((processed = mapper_->Process(range_scan)))
  {
    ...
    // ROS_INFO("SlamKarto::addScan checkpoint");
...
{
  ros::init(argc, argv, "slam_karto_gtsam_landmark");
