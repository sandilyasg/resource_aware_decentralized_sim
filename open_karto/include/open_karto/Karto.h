...
namespace karto
{
  ...
  /**
  * Defines a point (x, y) in 2-dimensional space for noting map feature positions.
  */
  class Point2
{
public:
  ...
  Point2()
  {
    // Default initialize the coordinates to (0.0, 0.0)
    m_Point = Vector2<kt_double>(0.0, 0.0);
  }

  /**
   * Constructor initializing point parameters
   * @param x x-coordinate
   * @param y y-coordinate
   */
  Point2(kt_double x, kt_double y)
  {
    m_Point = Vector2<kt_double>(x, y);
  }

  Point2(const Vector2<kt_double>& position)
  {
    m_Point = Vector2<kt_double>(position.GetX(), position.GetY());
  }
   ...
   * @return the position of the point as a vector
   */
   ...
   * @param rPosition the position of the point as a vector
   */
  ...
  /**
   * Return the squared distance between two Point2
   * @return squared distance
   */
  inline kt_double SquaredDistance(const Point2& rOther) const
  {
    return m_Point.SquaredDistance(rOther.m_Point);
  }
...
public:
   ...
   * Equality operator
   */
  inline bool operator==(const Point2& rOther) const
  {
    return (m_Point == rOther.m_Point);
  }
   ...
   * @return Point2 sum
   */
  inline Point2 operator+(const Point2& rOther) const
  {
    return Point2(m_Point + rOther.m_Point);
  }

  /**
   * Binary Point2 subtract
   * @param rOther
   * @return Point2 difference
   */
  inline Point2 operator-(const Point2& rOther) const
  {
    return Point2(m_Point - rOther.m_Point);
   ...
   * @param rStream input stream
   * @param rPoint to read
   */
  friend inline std::istream& operator>>(std::istream& rStream, const Point2& /*rPoint*/)
  {
    // Implement me!!
    return rStream;
  }
  ...
  /**
   * Write this point onto output stream
   * @param rStream output stream
   * @param rPoint to write
   */
  friend inline std::ostream& operator<<(std::ostream& rStream, const Point2& rPoint)
  {
    rStream << rPoint.m_Point.GetX() << " " << rPoint.m_Point.GetY();
    return rStream;
  }

private:
  Vector2<kt_double> m_Point;
}; // Point2


  /**
   * Type declaration of Point2 vector
   */
  ...
  public:
    ...
    /**
     * Sets the angle_min of this sensor_msgs/LaserScan Message
     * @param anglemin
     */
    inline void SetLaserScanAngleMin(float anglemin)
    {
     angle_min_ = double(anglemin);
    }

    inline void SetLaserScanAngleIncrement(float angleincrement)
    {
     angle_increment_ = double(angleincrement);
    }
    ...
    inline double GetAngleIncrement()
    {
